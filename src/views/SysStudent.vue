...
export default {
  ...
  methods: {
    ...
    updateData () {
      ...
      // fetch data
      this.$api.student.findPage(this.pageForm).then((res) => {
        this.data = res.data
    ...
    // CRUD
    create () {
      let promise = this.$api.student.save(this.submitForm)
      this.handlePromiseData(promise)
    },
    delete_ (id) {
      let promise = this.$api.student.deleteBook(this.tableData[id])
      this.handlePromiseData(promise)
    },
    update () {
      this.create()
